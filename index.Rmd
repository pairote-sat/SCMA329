...
::: {.example}
*A series of payments into and out of a bank account is
given as follows:*
...
::: {.example #egpv}
*An investor want to make a payment of ฿10000 in 2
...
value* of the payments. **Solution:** By the end of 2 years an initial
...
::: {.example}
*An investor deposits ฿10000 in a bank account that
...
::: {.example}
*An investor invests ฿1 at 7.5% p.a. (per annum)
...
::: {.example}
*An investor invests ฿1000 at 5.25% per half-year
...
::: {.example}
*An investor invests ฿1 at effective rate $i$% per time
...
::: {.example}
*An investor invests ฿200 at $3$% pa effective. What
...
the addition of interest as *accumulation* or *accumulating value*.
...
**Note** The model under the effective rate of interest condition is a
...
2.  *An investor invests ฿800 at $6$% per half-year effective. What will
...
::: {.example}
*Find the condition under which the two effective
...
::: {.example}
*The effective rate of interest per annum was 4%
...
::: {.example}
*The effective rate of interest per annum was 6%
...
*present value (PV)* or *discounted value* of the payments.
...
::: {.example}
*Calculate the present of ฿25000 due in 3 years at an
...
::: {.example}
*How much should we invest now to meet a liability of
...
::: {.example}
*Given the discount factor per year $v = 0.9$,
calculate*
...
::: {.example}
*Let $i$ be the constant effective rate of interest
...
::: {.example}
*Given the effective rate of interest of $8\%$ p.a.,
calculate*
...
::: {.example}
*Discount ฿2,000 for 3 years using the effective rate
...
::: {.example}
*The effective rate of discount $d$ per time unit can
...
::: {.example}
*Calculate the present value at time 0 of an annuity
...
::: {.example}
*Give the reason or show that the present value of a
...
::: {.example}
*Derive the formula for the present value of a simple
...
::: {.example}
*Assume that the effective rate of interest is 6% p.a.
...
::: {.example}
*Calculate the accumulation at 1 January 2020 of an
...
::: {.example}
*A nominal rate of interest of $i^{(4)} = 10\%$ p.a.
...
::: {.example}
*At a rate of 12% p.a. effective, draw a timeline to
...
::: {.example}
*At a rate of 12% p.a. compounding quarterly, draw a
...
::: {.example}
*Express each of the following effective rates per
...
::: {.example}
*Calculate the maximum price an investor wish to pay
...
::: {.example}
*Determine whether the following series of cashflows
...
::: {.example}
*An investor pays ฿1,000 in order to receive ฿600 back
...
::: {.example #exampleYield}
*An investor pays ฿1,000 in order to receive ฿300 back
...
::: {.example}
*Approximate the yield of the transaction in Example \@ref(exm:exampleYield).*
:::
...
::: {.example}
*You borrow ฿5,000 for a term of 3 years at a fixed
...
::: {.example}
*A loan of ฿20,000 is repayable by equal monthly
...
::: {.example}
*A person takes out a loan of ฿100,000 to be repaid by
...
::: {.example}
*You borrow ฿20,000 for a term of 20 years to be
...
::: {.example}
*You borrow ฿20,000 for a term of 15 years to be
...
::: {.example}
*Each bond of ฿100 nominal value carries coupons of 6%
pa payable half-yearly.*
:::
**Solution:** The coupon rate of 6% pa payable half-yearly means that bondholders will receive
$$ \frac{6\%}{2} \times 100 = ฿ 3 \text{ every half-year}.$$



::: {.example}
*An investor purchases ฿95 for a 5-year fixed interest
...
*A company issues a 10-year bond, to be redeemed at
...
*A tax-exempt investor buy the bond in Example \@ref(exm:exampleBondPrice) on its issue date. Calculate the price the
...
::: {.example}
*An investor liable to income tax at 30% buys a 15-year
...
::: {.example}
*An investor liable to the capital gains tax at 20%
purchases two bonds*
...
3.  *What price should the investor pay to obtain a yield of 9% pa effective?*   
:::


**Solution:**

1. The bond is redeemed at par. Therefore, the redemption amount is 100 THB which is greater than the bond price, and the capital gain tax is **payable**.
...
Working at time 10, the equation of value of the cashflows is 
$$ f(i) =  -96 (1+i)^{10} + 8 s^i_{\angl{10}} +  99.2 = 0.$$
By linear interpolation, we have
$$ f(0.08) = 7.836, \quad f(0.09) = -6.523, $$
and hence, the approximate of $i$ is $i^* \approx 0.08546 = 8.546\%$.

2. We know that $P = 96$ is equivalent to yield approximately equal to $8.546\%$. 
...
$$ P = 8 a^i_{\angl{10}} +  100\cdot (1+i)^{-10}, \quad i = 7\%,$$
which implies that $P = 107.02 > 100$. So our initial guess was correct and CGT is **not** payable.

3. Here $9\% > 8.546\%$. Let us guess that $P < 100$, i.e. CGT is payable.
$$ P = 8 a^i_{\angl{10}} +  99.2\cdot (1+i)^{-10}, \quad i = 7\%,$$
which implies that $P = 92.99 < 100$. So our initial guess was correct and CGT is payable.
...
::: {.example}
*A set of goods costs ฿98.25 in January 2013 and
...
Table: (\#tab:tableCPI) The monthly consumer price indices from
...
::: {.example}
*An investment contract made on January 2018 promises
...
::: {.example}
*An investor deposits 100 at time 0 and receives 120
after one year.*
...
::: {.example}
*A 5-year bond with nominal value of ฿100 was issued
...
2.  *in real terms with reference to the CPI.*
  ...
  Real value of cashflow at $t = 0$ | -100 | $\frac{(8)(98.25)}{100.15}$ | $\frac{(8)(98.25)}{99.74}$ | $\frac{(8)(98.25)}{99.21}$ | $\frac{(8)(98.25)}{100.75}$  | $\frac{(8)(98.25)}{101.44}$
  Real value of cashflow at $t = 0$   | -100 | = 7.85 | = 7.88 | = 7.91 | = 7.80 | = 104.60 
...
::: {.example}
*Consider an index-link bond of a nominal of ฿100
...
::: {#exampleILB .example}
*An investor purchased a 3-year index-linked bond in
...
::: {.example}
*Repeat Example\@ref(exm:exampleILB)
...
::: {#exampleILB2 .example}
*In January 2015, the government issued an
...
::: {.example}
*฿200 is invested in an account which pays a force of
interest of 4% pa. Calculate the amount in the account after 3 years.*
:::

::: {.example}
*A payment of ฿500 is due in 5 years' time. Calculate
...
::: {.example}
*A payment of ฿1000 is due in 1 years' time. Calculate
...
::: {.example}
*Calculate the expectation and variance of the random
variable $Z_i$.*
:::
...
::: {.example}
*We flip the coin once to represent each trading day.
...
::: {.example}
*Using Monte Carlo estimation, approximate the
...
::: {.example}
*Given that $W(0) = a$, what is the probability
...
::: {.example}
*Consider a Wiener process denoted by $W(t)$ with
...
::: {.example}
*Calculate the probability that the generalised Wiener
...
::: {.example}
*Consider the cash position of a company measured in
...
::: {.example}
*Explain why a generalised Wiener (GW) process is
...
::: {.example}
*Consider a stock that pays no dividends, provide an
...
::: {.example}
*Verify the calculation using the stock price
